...
use Acorn\SafetyHealthcheck\Content\SeedContent;
...
final class RulesEngineTest extends TestCase
{
    private function keys(array $overrides): array
    {
        ...
        foreach ($questions as &$q) { $q['variant_json'] = json_encode($q['question_key'] === 'M02_POLICY' ? ['under_5'=>'small','5_plus'=>'large'] : []); $q['applicability_json'] = json_encode(SeedContent::applicabilityForQuestion($q['question_key'])); }
    ...
    public function test_unsupported_operator_is_rejected(): void { $this->expectException(InvalidArgumentException::class); (new RulesEngine())->assertSupportedRule(['php' => 'system']); }
}
