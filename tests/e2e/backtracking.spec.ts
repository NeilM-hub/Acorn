...
test('online reload offers resume without storing contact data', async ({page}) => {
  ...
  await page.reload();
  await expect(page.getByRole('button', {name: 'Continue Healthcheck'})).toBeVisible();
...
test('network failure queues an answer and reconnect flushes it', async ({page, context}) => {
  ...
  await expect(page.getByText('Working offline — progress kept on this device')).toBeVisible();
...
test('Back restores the previous question and selected answer', async ({page}) => {
  ...
  await completeProfile(page);
  await page.getByLabel('Partly').check();
  const secondQuestion = await page.getByRole('heading', {level: 2}).textContent();
  await page.getByLabel('Yes').check();
  await page.getByRole('button', {name: 'Back'}).click();
  await expect(page.getByRole('heading', {level: 2})).toHaveText(secondQuestion || '');
...
test('editing profile removes a conditional module and its hidden answer', async ({page}) => {
  ...
  await page.getByLabel('Work at height').check();
  await page.getByLabel('Driving for work').check();
  ...
  await expect(page.getByRole('heading', {level: 2})).toContainText('work at height');
  await page.getByLabel('No').check();
  await page.getByRole('button', {name: 'Edit business profile'}).click();
  await page.getByLabel('Work at height').uncheck();
  await page.getByRole('button', {name: 'Continue to questions'}).click();
